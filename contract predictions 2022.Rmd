...
* **Kawhi Leonard** & **Chris Paul** both opted out of their player options & re-signed with their teams.
    + Kawhi signed a [4-year, `$176M` contract](https://www.nba.com/news/report-kawhi-leonard-plans-to-re-sign-with-clippers) with the Los Angeles Clippers while CP3 agreed to a [4-year `$120M` deal](https://www.cbssports.com/nba/news/nba-free-agency-2021-chris-paul-officially-re-signs-with-suns-on-four-year-deal/) with the Phoenix Suns
* Restricted free agents **John Collins** and **Jarrett Allen** cashed in, signing 5-year deals with their current teams.
    + Collins' deal with the Atlanta Hawks is worth [`$125M`](https://nba.nbcsports.com/2021/08/04/john-collins-re-signing-with-hawks-on-five-year-125m-contract/), while Allen got [`$100M`](https://bleacherreport.com/articles/2946293-jarrett-allen-re-signs-with-cavaliers-on-5-year-100m-contract-in-free-agency) from the Cleveland Cavaliers
* Restricted free agent **Lonzo Ball** and unrestricted free agents **DeMar DeRozan** & **Kyle Lowry** all switched teams via sign-and-trade
    + Lonzo ([4-year,`$80M`](https://www.sportsnet.ca/nba/article/bulls-officially-acquire-lonzo-ball-sign-trade-pelicans/)) & DeRozan ([3-year,`$85M`](https://www.espn.com/nba/story/_/id/31951447/chicago-bulls-demar-derozan-finalizing-sign-trade-deal)) joined forces on the Chicago Bulls with shooting guard Zach LaVine and center Nikola Vucevic. The two were former opponents in the Southwest division, with Lonzo a former member of the New Orleans Pelicans and DeRozan being a San Antonio Spur
    + Lowry joined up with Jimmy Butler & the Miami Heat on a [3-year,`$85M` contract](https://basketball.realgm.com/wiretap/263770/Kyle-Lowry-Signs-Three-Year-$85M-Deal-With-Heat) (coincidentally the same contract as his best friend DeRozan) after 9 seasons with the Toronto Raptors. That Raptors tenure included 6 All-Star appearances, franchise records in 3-point field goals made, assists and steals, and of course an NBA championship in 2019.
...
anti_join(current_fa,three_year_rolling_stats) %>% 
...
y1s2=kable(final_results %>% group_by(yrs_Y1S2) %>% tally())
s1y2=kable(final_results %>% group_by(yrs_S1Y2) %>% tally())
cat(c("\\begin{table}[h]",y1s2,"\\hspace{1cm} \\centering ",s1y2,"\\end{table}"))
```
...
For the 2022-2023 season, the salary cap is \$122,000,000. The minimum contract (from [RealGM](https://basketball.realgm.com/nba/info/salary_cap)) is \$1,616,044. Therefore the minimum first year percent of cap in 2021 is  `r formattable::percent(1616044/122000000,digits=2)`.
...
y1s2_correct=kable(final_results %>% group_by(yrs_Y1S2) %>% tally())
s1y2_correct=kable(final_results %>% group_by(yrs_S1Y2) %>% tally())
cat(c("\\begin{table}[h]",y1s2_correct,"\\hspace{1cm} \\centering ",
      s1y2_correct,"\\end{table}"))
```
...
final_results<-final_results %>%
  mutate(total_Y1S2=currency(total_Y1S2,digits=0),
         total_S1Y2=currency(total_S1Y2,digits=0),
         yr1_cap_percent_Y1S2=formattable::percent(yr1_cap_percent_Y1S2,digits=2),
         yr1_cap_percent_S1Y2=formattable::percent(yr1_cap_percent_S1Y2,digits=2))
```
...
options_decisions=inner_join(final_results,current_fa_options) %>%
  ...
  rename("Y1S2 Cap %"=yr1_cap_percent_Y1S2,
         ...
         "2021 Option"=option_amt) %>%
  select(player,"Y1S2 Cap %",yrs_Y1S2,total_Y1S2,"S1Y2 Cap %",yrs_S1Y2,
         total_S1Y2,"Option Type", "2021 Option")
kable(options_decisions) %>%
...
non_options %>% filter(type=="RFA") %>% select(-type) %>% slice(1:5) %>% 
...
non_options %>% filter(type=="RFA") %>% select(-type) %>% slice(6:10) %>% 
  kable() %>% kable_styling(latex_options="scale_down") %>%
kable_styling(latex_options = "HOLD_position")
```

The Martin twins (Cody stayed in Charlotte with the Hornets, while Caleb moved south to Miami) also have extremely similar projections, differing by 0.14% or \$`r round(0.0014*122.414,digits=2)`M at most!

On to the unrestricted free agents.
```{r ufas_top_five,echo=FALSE}
non_options %>% filter(type=="UFA") %>% select(-type) %>% slice(1:5) %>% 
  kable() %>% kable_styling(latex_options="scale_down") %>%
kable_styling(latex_options = "HOLD_position")
```
...
non_options %>% filter(type=="UFA") %>% select(-type) %>% slice(6:10) %>% 
  kable() %>% kable_styling(latex_options="scale_down") %>%
kable_styling(latex_options = "HOLD_position")
```
Monk was a bright spot in the Lakers' season, scoring 13.8 points per game on 47.3% shooting from the field, which were both career-highs. Tyus Jones provided a steady hand for the Memphis Grizzlies during Ja Morant's various injury absences, and might be looking for a team that can provide a more consistent starting role.
```{r ufas_last_five,echo=FALSE}
non_options %>% filter(type=="UFA") %>% select(-type) %>% slice(11:15) %>% 
  kable() %>% kable_styling(latex_options="scale_down") %>%
kable_styling(latex_options = "HOLD_position")
```
